...
### Messages
All messages can be send from an arbitrary source, which is indicated in the `from` field.  
Normally it's either `user` or `server`. 

The `type` property indicates what kind of message it is.
...
```json
{
  ...
  "type": "text",
  "text": "Hello world",
  "ssmlText": "<p>Hello <emphasis level=\"strong\">world</emphasis></p>"
}
```
The `ssmlText` is used to instruct speech synthesis and is optional.
...
```json
{
  ...
  "from:": "server",
  "type": "image",
  "url": "https://i.imgur.com/qOFRvNm.jpg",
  "ssmlText": "<p>A black and white kitten looking out of a window.</p>"
}
```
The `ssmlText` is used to instruct speech synthesis how to deal with this image and is optional.
...
```json
{
  ...
  "quickReplies": [
    ...
    },
    {
      "title": "I like this",
      "payload": "like",
      "imageUrl": "https://example.com/thumbs-up.png"
    },
    {
      "title": "Abort"
    }
...
```
Quick replies typically use a payload to send a different message to the bot than actually displayed to the user.  
If no payload is given, the title is used.

Add an image url, to display instead of the text.
...
```json
{
  ...
  "imageUrl": "https://example.com/abbey-road-cover.jpg",
  "clickUrl": "https://my-only-music-store.com",
  "buttons": [
    {
      "type": "postback",
      "text": "Buy",
      "value": "Buy Abbey Road"
    },
    {
      "type": "postback",
      "text": "Play",
      "value": "Play Abbey Road"
    }
  ]
}
```
The `subtitle`, `imageUrl`, `clickUrl` properties are all optional and subject to client support.

Use `buttons` to provide the user with options for responding. Even if not used, the field should be present as an empty array: `"buttons": []`.  
The only supported button type is `'postback'`. `value` is what is send to NLU and should be different to the display `text`, mainly to avoid visual duplication.
...
```
Query is the value send to an NLU for processing. Text can be a different value and is used for displaying messages in a client.
...
#### Typing message
Typing states can be `on` or `off`.
```json
{
  "$StaMP": true,
  "from:": "server",
